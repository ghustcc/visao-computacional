...
while True:
    ...
    # Verifique se a área da imagem cabe no frame
    ...
    cv2.rectangle(frame, (DANGER_ZONE[0], DANGER_ZONE[1]), (DANGER_ZONE[2], DANGER_ZONE[3]), (200, 0, 255), 2)      
    ...
    # ou no frame térmico, dependendo do que você quer que o modelo 'veja'.
    # Para a maioria dos modelos pré-treinados, é melhor detectar no frame colorido original
    # e depois desenhar no frame térmico.
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            dist_metros = calcular_distancia(centro_pessoa, centro_zona_perigo) / param
            
            # Definir um limite de segurança (ajuste conforme necessário)
            limite_seguranca = 200 # pixels
            limite_seguranca_maximo = 100 # pixels
            limite_seguranca_metros = limite_seguranca / param
            limite_maximo_metros = limite_seguranca_maximo / param
            
            status_perigo = ""
            cor_status = (255, 255, 255) # Branco
            
            if dist_metros < limite_seguranca_metros:
                status_perigo = "ALERTA: MUITO PROXIMO!"
                cor_status = (0, 0, 150) # Laranja
            elif dist_metros < limite_maximo_metros:
